...
import { searchAlbums, fetchAlbumDetails, searchSongs } from '../services/api';
...
const Home = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [trendingS, trendingA, latestA, hindi, punjabi, folk] = await Promise.all([
        searchSongs('Top Trending 2024', false),
        searchAlbums('Top Albums Hindi'),
        searchAlbums('New Hindi Albums 2024'),
        searchSongs('Latest Hindi', false),
        searchSongs('Latest Punjabi', false),
        searchSongs('Indian Folk', false)
      ]);
      ...
      setTrendingSongs(filterDuplicates(trendingS));
      setTrendingAlbums(trendingA || []);
      setLatestAlbums(latestA || []);
